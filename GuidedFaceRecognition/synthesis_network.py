...
class Decoder(nn.Module):
    def __init__(self, out_channels=3, dim=64, n_residual=3, n_upsample=2, latent_dim=8):
        ...
        self.mlp = MLP(latent_dim, num_adain_params)
...
class SubspaceLayer(nn.Module):
    def __init__(self, dim=128, n_basis=10):
        super().__init__()
    ...
    def forward(self, z):
        ...
        return z @ self.U + self.mu

class Generator(nn.Module):
    def __init__(self, in_channels=3, dim=64, n_residual=3, n_downsample=2, latent_dim=128, style_dim=10):
        super(Generator, self).__init__()
        self.encoder = Encoder(in_channels=in_channels, dim=dim, n_downsample=n_downsample)
        self.decoder = Decoder(out_channels=3, dim=dim, n_upsample=n_downsample, n_residual=n_residual, latent_dim=latent_dim)

        self.pca = SubspaceLayer(dim=latent_dim, n_basis=style_dim)

    def forward(self, x, style_z):

        z = self.pca(style_z)
        feat = self.encoder(x)
        out = self.decoder(feat, z)

        # regu
        UUT = self.pca.U @ self.pca.U.t()
        regu = ((UUT - torch.eye(UUT.shape[0], device=UUT.device)) ** 2).mean()
