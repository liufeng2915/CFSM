...
def main(args):
    ...
    train_loader = get_dataloader(
        cfg.rec, local_rank=args.local_rank, batch_size=cfg.batch_size, num_image=cfg.num_image, dali=cfg.dali)
    backbone = get_model(
    ...
    if cfg.resume:
        path_pfc = os.path.join(cfg.pretrained_partial_fc_path, "softmax_fc_gpu_{}.pt".format(rank))
        module_partial_fc.load_state_dict(torch.load(path_pfc))
        print('Successfully load pre-trained partial FC!')
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for data_idx, (img, local_labels) in enumerate(train_loader):
            ...
            loss: torch.Tensor = module_partial_fc(local_embeddings, updated_local_labels, opt)
        ...
        if rank == 0:
            path_module = os.path.join(cfg.output,"model_"+str(epoch)+".pt")
