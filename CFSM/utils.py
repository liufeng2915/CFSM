...
def sample_images(logs_dir, model_name, source_img, target_img, syn_img, epoch, data_idx):
    ...
    save_path = os.path.join(logs_dir,'samples',model_name)
    os.makedirs(save_path, exist_ok=True)

    save_image(
        source_img,
        os.path.join(save_path, str(epoch)+'_'+str(data_idx)+'_source.png'),
        nrow=8,
        normalize=True,
        range=(-1, 1),
    )
    save_image(
        target_img,
        os.path.join(save_path, str(epoch)+'_'+str(data_idx)+'_target.png'),
        nrow=8,
        normalize=True,
        range=(-1, 1),
    )
    save_image(
        syn_img,
        os.path.join(save_path, str(epoch)+'_'+str(data_idx)+'_syn.png'),
        nrow=8,
        normalize=True,
        range=(-1, 1),
    )
